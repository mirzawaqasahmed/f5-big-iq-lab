...
CONFIGURATION_TYPE="all"   #Configuration can be all,swg,access
UPGRADE_CONFIGURATION="false" #This flag set to true will make sure your existing configuration is not lost and only neccessary configuration
                             #will up added.

# Network Configuration - Modify these variables for your specific BIGIP
LOGIQ_SELFIP=10.192.116.41           # BIG-IQ self ip for log publication.
HOSTNAME=bigip-test14.lab.fp.f5net.com   # Hostname of BIGIP mgmt
SELF_IP_ADDR=10.192.116.189           # Self-IP Address of you BIGIP
VIRTUAL_IP=10.192.116.190           # Virtual address for traffic for swg, access reports

DEFAULT_ROUTE=10.192.123.1           # Default Route. If not sure, contact your network administrator.

SELF_IP_MASK=23                     # Self-IP Address Mask
VLAN_INTERFACE=1.1                  # Active interface on BIGIP for internal VLAN
VLAN_TAGGED=untagged                # Is VLAN tagged or untagged (values: tagged/untagged)
VLAN_TAG=4094                       # VLAN tag if VLAN is 'tagged'. Otherwise, no need to modify

DNS_SERVER=10.192.50.10 # You should modify this only if you are outside sjc dev lab. If outside sjc lab please check with you network administrator
NTP_SERVER=time.f5.com # No need to modify the NTP server. if outside sjc lab please check with you network administrator
...
tmsh create /ltm node /Common/remote_syslog_node { address $LOGIQ_SELFIP }
tmsh create /ltm pool /Common/remote_syslog_pool members add { /Common/remote_syslog_node:9997 { address $LOGIQ_SELFIP } }
...
tmsh create /sys log-config destination remote-syslog /Common/remote_rfc5424_destination { format rfc5424 remote-high-speed-log /Common/remote_syslog-hsl }
tmsh modify /sys log-config publisher /Common/remote_syslog_pub add destinations add {remote_rfc5424_destination}
