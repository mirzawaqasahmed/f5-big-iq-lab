...
DEFAULT_ROUTE=10.192.116.1           # Default Route. If not sure, contact your network administrator.
...
then
	...
	tmsh modify /apm profile access $SWG_POLICY_NAME  log-settings replace-all-with { default-log-setting }
fi
...
then
	...
	sleep 2
	tmsh modify /apm profile access $ACCESS_POLICY_NAME   log-settings replace-all-with { default-log-setting}
fi
...
then
	...
	tmsh create /ltm virtual /Common/swg_virtual destination $VIRTUAL_IP:$SWG_PORT ip-protocol tcp per-flow-request-access-policy $SWG_PER_REQUEST_POLICY_NAME  profiles add { http_swg  $SWG_POLICY_NAME { } classification { context clientside }} source-address-translation { type automap } source 0.0.0.0/0 
fi
