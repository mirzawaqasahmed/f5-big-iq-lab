...
echo -e "\n${RED}Did you subscribed and agreed to the software terms for F5 BIG-IP Virtual Edition - BEST - (BYOL) in AWS Marketplace?\n\n"
...
echo -e "\n${RED}In order to follow the AWS SSG creation, tail the following logs in BIG-IQ:\n/var/log/restjavad.0.log and /var/log/orchestrator.log${NC}\n"
...
echo -e "\n${RED}Application Creation: (it will start once AWS SSG creation is completed)\n${NC}"
python 09-create-aws-waf-app.py
echo -e "\n${BLUE}TIME:: $(date +"%H:%M")${NC}"

# add ab in crontab to simulate traffic
echo -e "\n${RED}Adding traffic generator in crontab.${NC}"
